{
  lib,
  config,
  ceedrichPkgs,
  ...
}: let
  poweroff = "custom/powermenu-poweroff";
  logout = "custom/powermenu-logout";
  lock = "custom/powermenu-lock";
  reboot = "custom/powermenu-reboot";
  suspend = "custom/powermenu-suspend";

  mkConfirm = label: cmd: ''${lib.getExe ceedrichPkgs.rofi-confirm-dialogue} "${label}" "${cmd}"'';
  wb = config.programs.waybar;
  cfg = wb.modules.powermenu;

  commands = config.logoutCommands;
in {
  options.programs.waybar.modules.powermenu = {
    name = lib.mkOption {
      type = lib.types.str;
      default = "group/powermenu";
      readOnly = true;
    };
    enable = lib.mkEnableOption "powermenu module";
    bars = lib.mkOption {
      default = [wb.mainBar];
      type = lib.types.listOf lib.types.str;
      description = "The names of the bars to add the module to";
    };
  };
  config.programs.waybar = lib.mkIf cfg.enable {
    settings = lib.genAttrs cfg.bars (bar: {
      ${cfg.name} = {
        orientation = "inherit";
        drawer = {
          transition-duration = 500;
          children-class = "not-power";
          # BUG: See https://github.com/Alexays/Waybar/issues/4382
          on-scroll-up = "true";
          on-scroll-down = "true";
        };
        modules = [poweroff logout lock suspend reboot];
      };

      ${poweroff} = rec {
        format = "󰐥";
        tooltip-format = "Poweroff";
        on-click = mkConfirm "${format} ${tooltip-format}" commands.shutdown;
        # BUG: See https://github.com/Alexays/Waybar/issues/4382
        on-scroll-up = "true";
        on-scroll-down = "true";
      };
      ${suspend} = {
        format = "󰏥";
        tooltip-format = "Suspend";
        on-click = commands.suspend;
        on-scroll-up = "true";
        on-scroll-down = "true";
      };
      ${logout} = rec {
        format = "󰍃";
        tooltip-format = "Logout";
        on-click = mkConfirm "${format} ${tooltip-format}" commands.logout;
        # BUG: See https://github.com/Alexays/Waybar/issues/4382
        on-scroll-up = "true";
        on-scroll-down = "true";
      };
      ${lock} = {
        format = "󰌾";
        tooltip-format = "Lock";
        on-click = commands.lock;
        # BUG: See https://github.com/Alexays/Waybar/issues/4382
        on-scroll-up = "true";
        on-scroll-down = "true";
      };
      ${reboot} = rec {
        format = "󰜉";
        tooltip-format = "Reboot";
        on-click = mkConfirm "${format} ${tooltip-format}" commands.reboot;
      };
    });
    style =
      #css
      ''
        #custom-powermenu-poweroff,
        #custom-powermenu-logout,
        #custom-powermenu-lock,
        #custom-powermenu-reboot,
        #custom-powermenu-suspend {
          color: @blue;
          padding: 0px 8px;
        }
      '';
  };
}
